...
export function AnkiNoteList({ notes }: AnkiNoteListProps) {
  ...
  return (
    <div className="space-y-3">
      {notes.map((note) => (
        <article key={note.note_id} className="rounded-md border border-border bg-card p-3">
          <div className="flex items-start justify-between gap-2">
            <h3 className="min-w-0 break-words text-sm font-semibold leading-snug">{note.word}</h3>
          ...
          {note.meaning && (
            <p className="mt-2 line-clamp-4 rounded-md bg-muted/70 p-2 text-[11px] leading-5 text-muted-foreground">
              {note.meaning}
            </p>
          )}
        </article>
